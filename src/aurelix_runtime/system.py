...
class SystemConfig:
    ...
    scheduler: SchedulerConfig = field(default_factory=lambda: SchedulerConfig(max_jobs_per_tick=1, max_attempts=3))
...
class AurelixSystem:
    ...
    @property
    def store(self): return self.runtime.store
    @property
    def status(self) -> str: return "stopping" if self._stop.is_set() else ("running" if self._started else "stopped")
    ...
    def submit(self, kind: str, payload: dict[str, str] | None = None) -> str: return self.runtime.submit(kind, payload or {})
    def _enqueue_due(self) -> int:
        now = time.monotonic(); count = 0
        with self._schedule_lock:
            for schedule in self.scheduler.schedules:
                if now < self._next_run.get(schedule.name, now + schedule.interval_seconds): continue
    ...
    def tick(self) -> list[str]:
        if not self._started: raise RuntimeError("system is not started")
        self._enqueue_due()
        return self.scheduler.tick()
    def start(self) -> None:
        if self._started: return
        self.scheduler.recover(); self._stop.clear(); self._started = True
        self.store.audit("system.started", "system", "system", "running", {})
    def stop(self) -> None:
        if not self._started: return
        self._stop.set(); self.scheduler.stop(); self.runtime.stop(); self._started = False
        self.store.audit("system.stopped", "system", "system", "stopped", {})
    def run_forever(self) -> None:
        self.start()
        try:
            while not self._stop.is_set():
                self.tick(); self._stop.wait(self.config.runtime.worker_poll_seconds)
        finally: self.stop()
