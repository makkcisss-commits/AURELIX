...
Runtime remains the execution authority. This coordinator only persists mission
identity and atomically reserves a new execution attempt after validated
learning; it never grants authorization.
"""
...
class MissionResumeCoordinator:
    """Small durable coordination boundary attached to the canonical RuntimeStore."""
    ...
    def __init__(self, store) -> None:
        ...
        with self.store.lock, self.store.db:
            ...
            self.store.db.execute(
                "CREATE INDEX IF NOT EXISTS idx_mission_state_active "
                "ON mission_state(active_execution_id)"
            )
        ...
        with self.store.lock:
            ...
            try:
                row = self.store.db.execute(
                    "SELECT * FROM mission_state WHERE mission_id=?", (mission_id,)
                ).fetchone()
                if row is not None:
                ...
                self.store.db.execute(
                    "INSERT INTO mission_state(mission_id,objective,required_capabilities,status,active_execution_id,resume_state,updated_at) VALUES(?,?,?,?,?,?,?)",
