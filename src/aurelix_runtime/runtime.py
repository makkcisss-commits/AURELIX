...
class RuntimeStore:
    """Durable SQLite state: jobs, audit events, approvals and runtime heartbeat."""
    ...
    def enqueue(self, kind: str, payload: dict[str, str]) -> str:
        job_id = str(uuid4()); now = datetime.now(timezone.utc).isoformat()
        with self.lock, self.db:
    ...
    def claim(self) -> Job | None:
        with self.lock, self.db:
            ...
            self.db.execute("UPDATE jobs SET status='running', attempts=attempts+1, updated_at=? WHERE job_id=?",
                            (now, row["job_id"]))
    ...
    def finish(self, job_id: str, success: bool, error: str | None = None) -> None:
        now = datetime.now(timezone.utc).isoformat()
        with self.lock, self.db:
            self.db.execute("UPDATE jobs SET status=?, updated_at=?, last_error=? WHERE job_id=?",
    ...
    def heartbeat(self) -> None:
        with self.lock, self.db:
            self.db.execute("INSERT OR REPLACE INTO runtime_state VALUES ('heartbeat', ?)",
                            (datetime.now(timezone.utc).isoformat(),))
    ...
    def status(self) -> dict[str, int | str]:
        with self.lock:
            ...
            return {"heartbeat": row[0] if row else "never", "queued": counts.get("queued", 0),
                    "running": counts.get("running", 0), "succeeded": counts.get("succeeded", 0),
                    "failed": counts.get("failed", 0)}


class AurelixRuntime:
    """24/7 orchestration loop with explicit handlers and durable state."""
    ...
    def run_once(self) -> bool:
        ...
        except Exception as exc:  # boundary: worker failure must not kill runtime
            self.store.finish(job.job_id, False, str(exc))
            self.store.audit("job.failed", "runtime", job.job_id, "failed", {"kind": job.kind, "error": str(exc)})
        return True

    def serve_forever(self) -> None:
        while not self._stop.is_set():
            worked = self.run_once()
            if not worked: self._stop.wait(self.config.worker_poll_seconds)
