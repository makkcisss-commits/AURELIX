...
class AutonomyFabric:
    ...
    def __init__(self, store: RuntimeStore | None = None, engine_store: EngineStore | None = None, research: ResearchEngine | None = None, academy: AcademyEngine | None = None, knowledge: KnowledgeEngine | None = None, innovation: InnovationEngine | None = None, experiment: ExperimentEngine | None = None, evaluation: EvaluationEngine | None = None, opportunity: OpportunityEngine | None = None, business: BusinessEngine | None = None, message_fabric: MessageFabric | None = None, capability_escalator: CapabilityEscalator | None = None, adaptive_loop: AdaptiveLoop | None = None, experiment_runner: ExperimentRunner | None = None, governor: Governor | None = None) -> None:
        self.store = store or RuntimeStore()
        if governor is None: governor = getattr(self.store, "_canonical_governor", None)
        if governor is None:
        ...
        self.academy = academy or AcademyEngine(); self.knowledge = knowledge or KnowledgeEngine(); self.innovation = innovation or InnovationEngine(); self.experiment = experiment or ExperimentEngine(); self.evaluation = evaluation or EvaluationEngine(); self.opportunity = opportunity or OpportunityEngine(); self.business = business or BusinessEngine(require_approval=True)
        self.capability_escalator = capability_escalator; self.adaptive_loop = adaptive_loop
    ...
    def _authorize_execution(self, objective: str, execution_id: str) -> str:
        ...
        if not decision.allowed:
            self.store.record_audit(execution_id, "autonomy.governor_denied", {"reason": decision.reason, "request_id": request.id}); raise PermissionError(f"autonomy execution denied by Governor: {decision.reason}")
        self.store.record_audit(execution_id, "autonomy.governor_allowed", {"request_id": request.id, "objective": objective}); return request.id
    ...
    def _collect_observations(self, experiment) -> list[dict[str, Any]]:
        with self.store.lock: rows = self.store.db.execute("SELECT observation FROM observations WHERE experiment_id=? ORDER BY recorded_at", (experiment.id,)).fetchall()
        return [json.loads(row[0]) for row in rows]

    def _persist_experiment(self, experiment, _run) -> None:
        self.engines.experiments[experiment.id] = experiment; self.engines.persist()
    ...
    def _capability_learning_result(self, claimed: JobRecord, required_capabilities: list[str], mission_id: str) -> AutonomyRun:
        ...
        if self.capability_escalator is None:
            self.store.record_audit(claimed.job_id, "autonomy.capability_escalation_unavailable", {"capabilities": unknown, "mission_id": mission_id}); status = "capability_escalation_unavailable"; academy = {"status": "blocked", "capability_gaps": unknown}
        else:
            for capability in unknown:
                ...
                else: gap, objective = self.capability_escalator.escalate(capability=capability, reason="required capability is not validated by the runtime", requested_by=claimed.worker_id or "autonomy")
                gaps.append({"gap_id": gap.gap_id, "capability": gap.capability, "study_objective_id": objective.objective_id}); self.store.record_audit(claimed.job_id, "autonomy.capability_escalated", {**gaps[-1], "mission_id": mission_id})
        ...
        self.store.complete(claimed.job_id, result, worker_id=claimed.worker_id, lease_token=claimed.lease_token); return AutonomyRun(**result)
    ...
    def run_claimed(self, claimed: JobRecord, required_capabilities: list[str] | None = None, mission_id: str | None = None) -> AutonomyRun:
        ...
        required_capabilities = required_capabilities or list(claimed.payload.get("required_capabilities", [])); persisted_mission_id = str(claimed.payload.get("mission_id", "")).strip(); mission_id = mission_id or persisted_mission_id or str(uuid4())
        ...
        mission.plan(list(DEFAULT_ECONOMIC_TASKS)); mission.start(); self._emit("mission.started", "orchestrator", execution_id, {"mission_id": mission_id, "objective": objective, "governor_request_id": governor_request_id})
        stop = threading.Event(); heartbeat = threading.Thread(target=self._heartbeat_loop, args=(execution_id, claimed.worker_id, claimed.lease_token, stop), name=f"aurelix-lease-{execution_id}", daemon=True); heartbeat.start(); self.store.record_audit(execution_id, "autonomy.started", {"objective": objective, "actor": claimed.worker_id, "mission_id": mission_id, "governor_request_id": governor_request_id})
        try:
            ...
            if experiment.get("experiment_id"):
                experiment_record = self.engines.experiments[experiment["experiment_id"]]; experiment_run = self.experiment_runner.execute(experiment_record); experiment = {"experiment_id": experiment_record.id, "status": experiment_run.status, "criteria": experiment_record.success_criteria, "result": experiment_record.result}
            ...
            self.store.complete(execution_id, result, worker_id=claimed.worker_id, lease_token=claimed.lease_token); self.store.record_audit(execution_id, "autonomy.completed", {"status": result["status"], "worker_id": claimed.worker_id, "mission_id": mission_id, "governor_request_id": governor_request_id}); self._emit("mission.completed", "orchestrator", execution_id, {"mission_id": mission_id, "status": lifecycle_status})
    ...
    def run(self, objective: str, execution_id: str | None = None, required_capabilities: list[str] | None = None, mission_id: str | None = None) -> AutonomyRun:
        execution_id = execution_id or str(uuid4()); mission_id = mission_id or str(uuid4()); job = self.store.enqueue("autonomy.run", {"objective": objective, "required_capabilities": required_capabilities or [], "mission_id": mission_id}, execution_id=execution_id); worker_id = f"autonomy:{execution_id}"; claimed = self.store.claim(job.job_id, worker_id=worker_id)
