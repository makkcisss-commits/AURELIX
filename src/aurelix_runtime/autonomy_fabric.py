...
class AutonomyFabric:
    ...
    def run_claimed(self, claimed: JobRecord, required_capabilities: list[str] | None = None, mission_id: str | None = None) -> AutonomyRun:
        ...
        mission_id = mission_id or str(claimed.payload.get("mission_id", "")) or str(uuid4())
    ...
    def resume_mission(self, blocked_execution_id: str) -> AutonomyRun:
        """Resume a blocked mission only after validated capabilities, preserving mission identity."""
        ...
        mission = self.adaptive_loop.resume_ready(blocked_execution_id)
        resume_execution_id = self._claim_resume_execution(blocked_execution_id, mission.mission_id)
    ...
    def _claim_resume_execution(self, blocked_execution_id: str, mission_id: str) -> str:
        ...
        resume_execution_id = f"{mission_id}:resume:{uuid4()}"
        with self.store.lock:
            ...
            try:
                ...
                if row is not None:
                    ...
                    if existing.get("execution_id"):
                        existing_id = str(existing["execution_id"])
                        if self.store.get(existing_id) is not None:
                            self.store.db.commit()
                            return existing_id
                self.store.db.execute("INSERT INTO runtime_state(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, json.dumps({"state": "reserved", "execution_id": resume_execution_id, "blocked_execution_id": blocked_execution_id, "mission_id": mission_id}, sort_keys=True)))
                self.store.db.commit()
    ...
    def _mark_resume_running(self, blocked_execution_id: str, execution_id: str) -> None:
        with self.store.lock, self.store.db:
            row = self.store.db.execute("SELECT value FROM runtime_state WHERE key=?", (f"mission-resume:{blocked_execution_id}",)).fetchone()
            if row is None:
                return
            state = json.loads(row[0]); state.update({"state": "running", "execution_id": execution_id})
            self.store.db.execute("UPDATE runtime_state SET value=? WHERE key=?", (json.dumps(state, sort_keys=True), f"mission-resume:{blocked_execution_id}"))
    ...
    def _mark_resume_completed(self, blocked_execution_id: str, execution_id: str) -> None:
        with self.store.lock, self.store.db:
            ...
            if row is None:
                return
            state = json.loads(row[0]); state.update({"state": "completed", "execution_id": execution_id})
            self.store.db.execute("UPDATE runtime_state SET value=? WHERE key=?", (json.dumps(state, sort_keys=True), f"mission-resume:{blocked_execution_id}"))
