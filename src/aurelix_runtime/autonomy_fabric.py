...
class AutonomyFabric:
    ...
    def __init__(self, store: RuntimeStore | None = None, engine_store: EngineStore | None = None,
        ...
        if governor is None:
            raise ValueError("autonomy fabric requires the canonical governor")
    ...
    def _authorize_execution(self, objective: str, execution_id: str) -> str:
        request = DecisionRequest(
            actor=Actor(id="autonomy-orchestrator", role="autonomy", autonomy=AutonomyLevel.A2),
            action=ActionClass.BUILD,
            reason="run canonical autonomous execution",
            payload={"objective": objective, "execution_id": execution_id},
        )
