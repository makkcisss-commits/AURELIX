...
class AdaptiveLoop:
    ...
    def validate_learning(self, *, execution_id: str, capability: str, objective_id: str, evaluation_id: str, evidence_refs: tuple[str, ...]) -> Any:
        ...
        return self.intelligence.validate_capability(name=capability, domain=objective.domain, required_competencies=objective.target_competencies, evidence_refs=evidence_refs)

    def capability_validated(self, capability: str) -> bool:
        normalized = capability.casefold()
        return any(item.name.casefold() == normalized and item.validated for item in self.intelligence.capabilities.values())
