...
class EconomicAttributionLedger:
    """Durable, observational ledger for externally verified economic outcomes."""
    ...
    def _load(self) -> None:
        ...
        with self.runtime_store.lock:
            row = self.runtime_store.db.execute("SELECT value FROM runtime_state WHERE key=?", (self._STATE_KEY,)).fetchone()
        if row is None:
            return
    ...
    def _persist(self) -> None:
        ...
        payload = {
            key: {
                **{name: value for name, value in asdict(entry).items() if name not in {"expected_daily_eur", "observed_daily_eur", "variance_daily_eur"}},
