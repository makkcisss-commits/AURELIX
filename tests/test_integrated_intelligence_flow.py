...
class FakeModelProvider(ModelProvider):
    ...
    def structured_output(self, prompt: str, schema: dict):
        return {"title": "Evidence-backed automation opportunity", "problem": "A validated workflow remains manual.", "proposed_solution": "Automate the bounded workflow.", "expected_value": "Reduce cycle time.", "estimated_cost": 100.0, "risk": 2, "confidence": 0.8}

    def embeddings(self, text: str) -> list[float]: return [1.0, 0.0]
...
def test_research_knowledge_innovation_experiment_evaluation(tmp_path: Path):
    ...
    def research_provider(query: str): return [Evidence("https://example.test/source", f"Source-backed finding for {query}", 0.9, True)]
...
def test_complete_intelligence_flow_and_execution(tmp_path: Path):
    ...
    def research_provider(query: str): return [Evidence("https://example.test/source", f"Verified finding for {query}", 0.95, True)]
    ...
    assert result["opportunity"]["status"] == "candidate"
