...
def _opportunity():
    ...
    evidence_by_claim = {
        ...
        for claim, ref in (
            ("demand", "prospect-1"),
            ("monetization_path", "offer-1"),
            ("source_reality", "source-1"),
        )
    }
...
def test_external_reference_is_idempotent_with_durable_ledger(tmp_path):
    ...
    ledger = DurableRevenueLedger(store, engine)
    bridge = OpportunityRevenueBridge(ledger)
    opportunity, qualification = _opportunity()
    source = bridge.admit(opportunity, qualification=qualification, owner_role="business", channel="test")

    first = bridge.record_observation(source.source_id, Decimal("10"), external_reference="payment-123")
    second = bridge.record_observation(source.source_id, Decimal("10"), external_reference="payment-123")

    assert first.observed_daily_eur == Decimal("10")
    assert second.observed_daily_eur == Decimal("10")
    assert ledger.total_for_activity(source.source_id) == Decimal("10")
    store.close()
