...
def test_capability_block_is_resumable_with_same_mission_id() -> None:
    ...
    result = factory.autonomy_fabric.run("resume a blocked mission", execution_id=execution_id, required_capabilities=["crm-write"])
    ...
    evidence = factory.adaptive_loop.record_evidence(objective_id=objective.objective_id, kind=EvidenceKind.EXPERIMENT, reference="test://crm-write/resume-proof", strength=1.0)
    factory.continuous_intelligence.validate_capability(name="crm-write", domain=objective.domain, required_competencies=objective.target_competencies, evidence_refs=(evidence.evidence_id,))
...
def test_unvalidated_capability_cannot_resume() -> None:
    ...
    result = factory.autonomy_fabric.run("do not resume an unvalidated mission", execution_id=execution_id, required_capabilities=["crm-write"])
    assert result.status == "capability_learning_required"
    try:
