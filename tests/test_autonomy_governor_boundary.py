...
def test_autonomy_execution_is_blocked_by_governor(tmp_path):
    ...
    denied = next(row for row in runtime.store.audit_summary(20)["recent"] if row["event_type"] == "autonomy.governor_denied")
    record = runtime.store.get(denied["job_id"])
    assert record is not None
    assert record.status == "failed"
    runtime.close()


def test_autonomy_fabric_reuses_one_governor_per_runtime_store(tmp_path):
    store = RuntimeStore(tmp_path / "runtime.db")
    first = AutonomyFabric(store=store)
    second = AutonomyFabric(store=store)
    assert first.governor is second.governor
    first.close()
